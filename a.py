...
def _extract_sheet_safely(
    ...
    def _structured() -> pd.DataFrame:
        return extract_and_flatten_sheet(file_bytes, sheet_name, preferred_profile=preferred_profile)

    def _auto_flat() -> pd.DataFrame:
        quick_df = _skip_error_heavy_rows(
            pd.read_excel(io.BytesIO(file_bytes), sheet_name=sheet_name, header=None),
            max_check=50,
        )
        if quick_df is None or quick_df.empty:
            return pd.DataFrame()
        auto_df = auto_flatten_report_tables(quick_df, extraction_profile=preferred_profile)
        if auto_df is None or auto_df.empty:
            return pd.DataFrame()
        return finalize_extracted_sheet(auto_df, strip_text=True, split_hierarchy=True)
    ...
    strategies = (
        ("Structured extraction", _structured),
        ("Auto-flat fallback", _auto_flat),
        ("Flat fallback", _flat),
    )
    for description, strategy in strategies:
        try:
            ...
            print(f"[extract_sheet] {description} failed: {e}")
            continue
...
def initialize_session_state():
    ...
    setdefault also backfills keys added by later app versions, so sessions
    started before an update pick up new state without losing existing values.
    """
    defaults = {
        'initialized': False,
        'azure_client': None,
        ...
        'query_history': [],
        'graph_memory': [],
        'use_graph_orchestration': True,
        'graph_thread_id': hashlib.sha256(str(datetime.now()).encode()).hexdigest()[:12],
        'uploaded_files': [],
        'files_loaded': False,
        'file_sheets': {},           # Maps file names to their sheets
        'selected_sheets': {},       # Maps file names to selected sheets
        'flat_file_overrides': {},   # Maps Excel file names to 'already flat' boolean
        'show_sheet_selector': False,
        'pending_clarified_question': "",
    }
...
def analyze_uploaded_files(uploaded_files: list[Any] | None):
    ...
    for uploaded_file in uploaded_files:
        ...
        if file_extension in ['.xlsx', '.xls']:
            ...
            if _workbook_archive_rejected(file_bytes, file_name):
                continue
...
def _get_orchestrator() -> SQLAgentOrchestrator:
    """The session's orchestrator, created on first use and seeded with graph memory."""
...
def _workbook_archive_rejected(file_bytes: bytes, file_name: str, prefix: str = "") -> bool:
    """True when the workbook fails the zip-archive safety check (and reports it)."""
    try:
        _validate_workbook_archive(file_bytes)
    except ValueError as exc:
        st.error(f"{prefix}'{file_name}' was rejected: {exc}")
        return True
    except Exception:
        pass  # non-zip (e.g. legacy .xls); let pandas decide downstream
    return False


def _sheets_to_process(file_bytes: bytes, file_name: str, selected_sheets) -> list[str]:
    """Sheets chosen for this file, defaulting to every sheet in the workbook."""
    if selected_sheets and file_name in selected_sheets:
        return selected_sheets[file_name]
    return get_excel_sheets_from_bytes(file_bytes)


def _record_loaded_sheets(files_db, file_name: str, mappings: list[str], success_label: str) -> bool:
    """Register a file's loaded sheets and report the outcome. True if anything loaded."""
    if not mappings:
    ...
    st.success(f"✅ {success_label}: {file_name} ({len(mappings)} sheet(s))")
    return True
    ...
    try:
        with st.spinner("Loading files..."):
            ...
            for uploaded_file in uploaded_files:
                ...
                if file_extension in ['.xlsx', '.xls']:
                    if _workbook_archive_rejected(file_bytes, file_name, prefix="❌ "):
                        continue
                    sheets_to_process = _sheets_to_process(file_bytes, file_name, selected_sheets)
                ...
                if file_extension in ['.xlsx', '.xls'] and force_flat:
                    ...
                    loaded_count += _record_loaded_sheets(
                        files_db, file_name, loaded_sheet_mappings, "Loaded flat file"
                    )
                    continue
                ...
                if file_extension in ['.xlsx', '.xls'] and not force_flat:
                    ...
                    for sheet_name in sheets_to_process:
                        ...
                        if structured_df is None or structured_df.empty:
                            ...
                            structured_df = finalize_extracted_sheet(structured_df, strip_text=True,
                    ...
                    loaded_count += _record_loaded_sheets(
                        files_db, file_name, loaded_sheet_mappings, "Loaded"
                    )
                    continue
...
def load_uploaded_files(uploaded_files, selected_sheets=None, flat_file_overrides=None):
    ...
    if uploaded_files:
        ...
        sql_agent = _get_orchestrator().build_agent(
            files_db,
    ...
    if st.session_state.files_db and st.session_state.files_db.tables_info:
        ...
        for table_name, info in st.session_state.files_db.tables_info.items():
            with st.expander(f"📄 **{table_name}** - {info['row_count']:,} rows"):
                ...
                st.markdown("---")
                st.caption(
                    "Use advanced cleanup to rename columns, split hierarchy values, and re-apply cleaned data to this in-memory table.")
                        ...
                        if final_df is not None and not final_df.empty:
                            ...
                            with apply_col:
                                if st.button(
                                    ...
                                    replaced_table = _get_orchestrator().replace_table(
                                        st.session_state.files_db,
                                        table_name,
                                        final_df,
...
def execute_query(user_question: str):
    ...
    with st.spinner("Generating SQL and executing query..."):
        result = _get_orchestrator().run_free_text_query(
...
# Row height / header height used to size a dataframe so every row is visible
# without an inner scrollbar. Beyond MAX_FULL_TABLE_HEIGHT the frame scrolls,
# otherwise a large result would render a page tens of thousands of pixels tall.
_DF_ROW_PX = 35
_DF_HEADER_PX = 38
MAX_FULL_TABLE_HEIGHT = 3000


def _full_table_height(row_count: int) -> int:
    """Height that shows every row, capped so huge results stay usable."""
    return min(_DF_HEADER_PX + _DF_ROW_PX * max(int(row_count), 1), MAX_FULL_TABLE_HEIGHT)


def _render_full_table(df: pd.DataFrame, scope: dict | None = None) -> None:
    """Render a result table in full, captioned with its row count and scope."""
    height = _full_table_height(len(df))
    st.dataframe(df, use_container_width=True, hide_index=True, height=height)

    caption = f"📊 {len(df):,} row(s) × {len(df.columns)} column(s)"
    scope_text = " · ".join(
        f"{name}: {', '.join(str(v) for v in values)}"
        for name, values in (scope or {}).items() if values
    )
    if scope_text:
        caption += f" — scope: {scope_text}"
    if height >= MAX_FULL_TABLE_HEIGHT:
        caption += " (scroll for the remaining rows)"
    st.caption(caption)


# Marker used to recognise (and replace) a previous "did you mean" refinement
# so repeated corrections don't stack up inside the question.
_SUGGESTION_MARKER = "matched no rows. Use"


def _refined_question_from_suggestion(result: dict, column: str, value: str) -> str:
    """Rewrite the question to use a real value instead of the one that missed."""
    base = str(result.get("question") or "").strip()
    # Drop any earlier suggestion line so corrections replace rather than pile up.
    base = "\n".join(
        line for line in base.splitlines() if _SUGGESTION_MARKER not in line
    ).strip()

    missed = str((result.get("value_suggestions") or {}).get("value") or "").strip()
    correction = (
        f'"{missed}" {_SUGGESTION_MARKER} "{value}" for {column} instead.'
        if missed else f'Use "{value}" for {column}.'
    )
    return "\n\n".join(part for part in (base, correction) if part)


def _render_value_suggestions(result: dict) -> bool:
    """Offer real column values as buttons when a filter matched nothing.

    Returns True if anything was rendered. Clicking re-runs the query through
    the same pending-question path the clarification buttons use.
    """
    suggestions = result.get("value_suggestions") or {}
    alternatives = suggestions.get("alternatives") or []
    column = str(suggestions.get("column") or "")
    missed = str(suggestions.get("value") or "")
    if not alternatives or not column:
        return False

    st.markdown(
        f'No rows matched **{column} = "{missed}"**, but these real values exist:'
        if missed else f"Real values available in **{column}**:"
    )
    for value in alternatives:
        key = "suggest_" + hashlib.sha256(
            f"{result.get('question', '')}|{column}|{value}".encode()
        ).hexdigest()[:12]
        if st.button(f'Use "{value}" instead', key=key, use_container_width=True):
            st.session_state.pending_clarified_question = _refined_question_from_suggestion(
...
def _clarification_options(result: dict):
    """Yield (index, option, label) for a clarification result; label may be ''."""
    options = result.get('clarification_options', [])
    labels = result.get('option_labels', [])
    for i, opt in enumerate(options, 1):
        label = labels[i - 1] if i - 1 < len(labels) and labels[i - 1] else ""
        yield i, opt, label
...
def display_query_result(result: dict):
    ...
    has_financial_guardrail = any(_warning_is_financial_guardrail(w) for w in warnings_list)
    ...
    with result_container:
        ...
        if needs_clarification:
            pass  # the clarification callout below acts as the banner
        elif result['success']:
        ...
        if needs_clarification:
            ...
            if result.get('clarification_options'):
                ...
                for i, opt, label in _clarification_options(result):
                    st.markdown(
                        f"&nbsp;&nbsp;**{i}. {label}:** {opt}" if label
                        else f"&nbsp;&nbsp;**{i}.** {opt}"
                    )
                    ...
                    if st.button(
                        f"Use option {i}" + (f": {label}" if label else ""),
                        key=button_key,
                        use_container_width=True,
                    ):
            ...
            # Escape hatch for the scope drill-down only. It has no meaning for
            # an LLM clarification ("which metric did you mean?"), where there
            # is no scope to widen.
            is_scope_question = (
                (result.get('interpreted_intent') or {}).get('route') == 'scope_clarification'
            )
            if is_scope_question and st.button(
                "⏭️ Answer anyway (combine all scopes)",
                key="scope_answer_anyway_" + hashlib.sha256(
                    str(result.get('question', '')).encode()
                ).hexdigest()[:12],
                use_container_width=True,
                help="Runs the query without narrowing further. The result will "
                     "say which scopes were mixed together.",
            ):
                st.session_state.pending_clarified_question = (
        ...
        if result['success']:
            ...
            if results_df is not None and len(results_df) > 0:
                ...
                # Single-value result → headline it, then still show the table.
                if len(results_df) == 1 and len(results_df.columns) == 1:
                    ...
                    scope_suffix = " · ".join(
                        ...
                        for values in result_scope.values() if values
                    )
                    st.markdown(
                        f"**{results_df.columns[0]}:** {formatted}"
                        + (f"  \n*{scope_suffix}*" if scope_suffix else "")
                    )
                _render_full_table(results_df, result_scope)
            ...
            # A filter that matched nothing (or summed to zero) can be corrected
            # in one click, for empty and single-zero results alike.
            _render_value_suggestions(result)
        ...
        for idx, result in enumerate(st.session_state.query_history):
            ...
            with st.expander(
                ...
                if result.get('needs_clarification'):
                    ...
                    if result.get('clarification_options'):
                        parts = [
                            f"**{i}. {label}:** {opt}" if label else f"**{i}.** {opt}"
                            for i, opt, label in _clarification_options(result)
                        ]
                ...
                # ── Success entry ───────────────────────────────────────
                    ...
                    if results_df is not None and len(results_df) > 0:
                        st.markdown("**Results:**")
                        _render_full_table(results_df, result.get('result_scope'))
...
def main():
    ...
    with st.sidebar:
        ...
        uploaded_file_list = uploaded_files if isinstance(uploaded_files, list) else (
        ...
        if uploaded_file_list:
                ...
                with st.spinner("Analyzing files..."):
                    ...
                    for file_name, info in file_sheets.items():
                        if info['type'] == 'excel' and info['sheets']:
                            st.session_state.selected_sheets[file_name] = existing_selected.get(file_name,
                                                                                                info['sheets'].copy())
                            st.session_state.flat_file_overrides[file_name] = bool(
                                existing_overrides.get(file_name, False))
            ...
            if st.session_state.show_sheet_selector and st.session_state.file_sheets:
                ...
                for file_name, info in st.session_state.file_sheets.items():
                    if info['type'] == 'excel' and info['sheets']:
                        ...
                        with st.expander(f"📁 {file_name}", expanded=True):
                            ...
                            for column, label, key_prefix, select in (
                                    (select_all_col, "✅ Select All", "select_all", True),
                                    (clear_all_col, "❌ Clear All", "clear_all", False),
                            ):
                                with column:
                                    if st.button(label, key=f"{key_prefix}_{file_name}",
                                                 use_container_width=True):
                                        st.session_state.selected_sheets[file_name] = (
                                            info['sheets'].copy() if select else []
                                        )
                                        # Force widget state update by clearing checkbox keys
                                        for sheet in info['sheets']:
                                            widget_key = f"sheet_{file_name}_{sheet}"
                                            if widget_key in st.session_state:
                                                st.session_state[widget_key] = select
                                        st.rerun()
                ...
                st.markdown("---")
                st.info(
                    "ℹ️ Excel sheets are auto-flattened by default. You can mark specific files as already flat to skip extraction.")
                ...
                if st.button(f"🔄 {action_label} ({total_selected} sheet(s) selected)", type="primary",
                             use_container_width=True):
                    with st.spinner("Loading files..."):
