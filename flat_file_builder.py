...
def _parse_numeric_text_for_detection(text: str) -> float | None:
    ...
    token = str(text or "").strip()
    if not token or _looks_like_date_or_period_text(token):
        return None
...
def is_unit_label(value) -> bool:
    return bool(RE_UNIT_LABEL.fullmatch(cell_text(value)))
...
def _build_sheet_scan_context(raw_df: pd.DataFrame) -> SheetScanContext:
    ...
    for idx, value in enumerate(flat_values):
        ...
        text_flat[idx] = text
        unit_like = is_unit_label(text)
        unit_flat[idx] = unit_like

        if isinstance(value, (int, float, np.integer, np.floating)) and not isinstance(value, bool):
            numeric_like = not bool(np.isnan(value)) if isinstance(value, (float, np.floating)) else True
        else:
            numeric_like = _parse_numeric_text_for_detection(text) is not None
        numeric_flat[idx] = numeric_like

        nm_like = text.lower() in {"n.m.", "n.m", "nm"}
        data_like_flat[idx] = (numeric_like or nm_like) and not unit_like
...
def _region_has_table_signal(region_df: pd.DataFrame) -> bool:
    if region_df is None or region_df.empty:
        return False
    if region_df.shape[0] < 3 or region_df.shape[1] < 2:
        return False
...
def _apply_sheet_level_context(
    ...
    if (
        ...
        for merged in merged_ranges:
            ...
            child_ids = governed["table_id"].map(cell_text).drop_duplicates().tolist()
...
def _drop_numeric_pipe_spillover_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "source_cell" in df.columns:
        trusted_source = _series_cell_text(df["source_cell"]).ne("").to_numpy(dtype=bool)
        spillover_mask &= ~trusted_source
    ...
    def _numeric_mask(series: pd.Series) -> pd.Series:
        return series.map(
            lambda value: (
                False
                if pd.isna(value)
                else _parse_numeric_text_for_detection(str(value)) is not None
            ),
            na_action=None,
        ).astype(bool)
...
def _rescue_financial_value_cells(raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for cluster_idx, rows in cluster_rows.items():
        ...
        if header_rows and value_cols:
            cluster_headers[cluster_idx] = _composite_headers_for_region(
                raw_df,
                header_rows,
                value_cols,
            )
        else:
            cluster_headers[cluster_idx] = ({}, {})
...
def _merge_extraction_with_coverage_rescue(
    ...
    if "source_cell" not in extracted.columns:
        return pd.concat([extracted, rescued], ignore_index=True, sort=False)
...
def infer_flat_table_structure(
    ...
    _, report_reason, has_report_signals, has_matrix_signals = infer_report_layout_from_quick_df(
        quick_df
    )
    if has_matrix_signals:
...
def _read_display_sheet_cached(file_bytes: bytes, sheet_name: str) -> pd.DataFrame:
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(file_bytes)) as archive:
            for member in archive.namelist():
                ...
                # exact unprefixed opening tag.
                if b"mergeCell" in archive.read(member):
    ...
    workbook = openpyxl.load_workbook(
        io.BytesIO(file_bytes),
        read_only=not has_merged_cells,
        data_only=True,
        keep_vba=False,
        rich_text=False,
    )
...
SQL_AGENT_MAX_DETAIL_ROWS = 200
SQL_AGENT_FORBIDDEN_SQL = re.compile(
    r"\b(?:INSERT|UPDATE|DELETE|DROP|ALTER|CREATE|REPLACE|TRUNCATE|ATTACH|DETACH|"
    r"VACUUM|REINDEX|ANALYZE|PRAGMA|GRANT|REVOKE)\b",
    re.IGNORECASE,
)
...
def validate_sql_agent_query(query: str) -> tuple[bool, str]:
    """Conservative validation for model-generated read-only SQLite queries."""
    ...
    if SQL_AGENT_FORBIDDEN_SQL.search(text):
        return False, "Only read-only SQL is allowed."
    first_token = re.match(r"^[\s(]*(\w+)", text)
    if not first_token or first_token.group(1).upper() not in {"SELECT", "WITH", "EXPLAIN"}:
        return False, "Query must begin with SELECT, WITH, or EXPLAIN."
    return True, ""


def _sql_agent_ddl() -> str:
    return """
PRAGMA foreign_keys = ON;

CREATE TABLE agent_workbooks (
...
CREATE TABLE agent_sheets (
    ...
    workbook_id TEXT NOT NULL REFERENCES agent_workbooks(workbook_id),
    sheet_name TEXT NOT NULL,
    sheet_index INTEGER NOT NULL,
    parsing_mode TEXT NOT NULL,
    source_row_count INTEGER,
    source_column_count INTEGER,
    logical_table_count INTEGER NOT NULL,
    observation_count INTEGER NOT NULL
);

CREATE TABLE agent_parent_reports (
    parent_report_id TEXT PRIMARY KEY,
    sheet_id TEXT NOT NULL REFERENCES agent_sheets(sheet_id),
    report_name TEXT NOT NULL,
    description TEXT NOT NULL,
    child_table_count INTEGER NOT NULL,
    search_terms_json TEXT NOT NULL
);

CREATE TABLE agent_logical_tables (
    logical_table_id TEXT PRIMARY KEY,
    sheet_id TEXT NOT NULL REFERENCES agent_sheets(sheet_id),
    parent_report_id TEXT REFERENCES agent_parent_reports(parent_report_id),
    ...
    non_additive_dimensions_json TEXT NOT NULL,
    requires_unit_filter INTEGER NOT NULL,
    known_units_json TEXT NOT NULL,
    known_sections_json TEXT NOT NULL,
    search_terms_json TEXT NOT NULL
);

CREATE TABLE excel_observations (
    observation_id TEXT PRIMARY KEY,
    workbook_id TEXT NOT NULL REFERENCES agent_workbooks(workbook_id),
    sheet_id TEXT NOT NULL REFERENCES agent_sheets(sheet_id),
    logical_table_id TEXT NOT NULL REFERENCES agent_logical_tables(logical_table_id),
    table_subtitle TEXT,
    section TEXT,
    column_group TEXT,
    shared_header TEXT,
...
CREATE TABLE observation_provenance (
    observation_id TEXT PRIMARY KEY REFERENCES excel_observations(observation_id),
    source_range TEXT,
    source_cell TEXT,
    source_row INTEGER,
    source_column TEXT,
...
CREATE TABLE agent_columns (
    ...
    sample_values_redacted INTEGER NOT NULL,
    ...
    allowed_aggregations_json TEXT NOT NULL,
    non_additive_dimensions_json TEXT NOT NULL
);
...
CREATE TABLE agent_verified_queries (
    verified_query_id TEXT PRIMARY KEY,
...
CREATE INDEX idx_observations_logical_table ON excel_observations(logical_table_id);
CREATE INDEX idx_observations_sheet ON excel_observations(sheet_id);
CREATE INDEX idx_observations_line_item ON excel_observations(line_item);
CREATE INDEX idx_observations_metric ON excel_observations(metric);
CREATE INDEX idx_observations_period ON excel_observations(period, metric_date);
CREATE INDEX idx_observations_unit ON excel_observations(unit);

CREATE VIEW v_excel_facts AS
SELECT
    o.observation_id,
    w.file_name,
    s.sheet_name,
    t.logical_table_id,
    t.parent_report_id,
    p.report_name AS parent_report_name,
    t.logical_name,
    ...
    o.value_numeric,
    o.value_kind
FROM excel_observations o
JOIN agent_logical_tables t ON t.logical_table_id = o.logical_table_id
LEFT JOIN agent_parent_reports p ON p.parent_report_id = t.parent_report_id
JOIN agent_sheets s ON s.sheet_id = o.sheet_id
JOIN agent_workbooks w ON w.workbook_id = o.workbook_id;

CREATE VIEW v_agent_table_catalog AS
SELECT
    t.logical_table_id,
    t.parent_report_id,
    p.report_name AS parent_report_name,
    ...
    t.additivity,
    t.requires_unit_filter,
    t.known_units_json,
    t.known_sections_json,
    t.search_terms_json
FROM agent_logical_tables t
LEFT JOIN agent_parent_reports p ON p.parent_report_id = t.parent_report_id
JOIN agent_sheets s ON s.sheet_id = t.sheet_id
JOIN agent_workbooks w ON w.workbook_id = s.workbook_id;

CREATE VIEW v_agent_column_catalog AS
SELECT
    t.logical_name,
    c.*
FROM agent_columns c
JOIN agent_logical_tables t ON t.logical_table_id = c.logical_table_id;

CREATE VIEW v_agent_metric_catalog AS
SELECT
    t.logical_name,
    m.*
FROM agent_metrics m
JOIN agent_logical_tables t ON t.logical_table_id = m.logical_table_id;
""".strip()


def _insert_sqlite_rows(connection: sqlite3.Connection, table_name: str, frame: pd.DataFrame) -> None:
...
def _agent_column_sql_type(column: str, series: pd.Series) -> str:
...
def _agent_column_role(column: str) -> str:
    if column == "value_numeric":
        return "fact"
    if column == "value_text":
        return "raw_fact"
    if column == "value_kind":
    ...
    if column in {"parent_line_item", "line_item_path", "indent_level", "line_item"}:
        return "hierarchy_dimension"
    if column in {"workbook_id", "sheet_id", "logical_table_id", "observation_id"}:
        return "key"
    return "dimension"


def _build_agent_column_rows(
    ...
    for column in table_df.columns:
        ...
        text_values = [_sql_text(value) for value in series.tolist()]
        non_blank = [value for value in text_values if value]
        distinct = list(dict.fromkeys(non_blank))
        redacted = _is_sensitive_agent_column(str(column))
        samples = [] if redacted else distinct[:max_sample_values]
        role = _agent_column_role(str(column))
        if column == "value_numeric":
            allowed_aggregations = ["SUM", "AVG", "MIN", "MAX", "COUNT"]
            non_additive_dimensions = aggregation_policy["non_additive_dimensions"]
        else:
            allowed_aggregations = ["COUNT", "COUNT_DISTINCT"] if role.endswith("dimension") else []
            non_additive_dimensions = []
        rows.append(
            {
                "column_id": _stable_agent_id("col", logical_table_id, str(column)),
                "logical_table_id": logical_table_id,
                "column_name": str(column),
                "sql_type": _agent_column_sql_type(str(column), series),
                "semantic_role": role,
                "description": SQL_AGENT_COLUMN_DESCRIPTIONS.get(
                    str(column),
                    f"Dimension extracted from the workbook: {str(column).replace('_', ' ')}.",
                ),
                "synonyms_json": json.dumps(SQL_AGENT_COLUMN_SYNONYMS.get(str(column), []), ensure_ascii=False),
                "sample_values_json": json.dumps(samples, ensure_ascii=False),
                "sample_values_redacted": int(redacted),
                "nullable": int(len(non_blank) < len(series)),
                "is_unique": int(bool(non_blank) and len(distinct) == len(non_blank)),
                "is_enum": int(0 < len(distinct) <= 20 and len(distinct) < max(len(series) * 0.5, 2)),
                "is_filterable": int(role in {"dimension", "time_dimension", "hierarchy_dimension", "fact_metadata"}),
                "is_hidden": int(role == "key" or str(column) == "value_text"),
                "fill_rate": round(len(non_blank) / max(len(series), 1), 4),
                "distinct_count": len(distinct),
                "allowed_aggregations_json": json.dumps(allowed_aggregations),
                "non_additive_dimensions_json": json.dumps(non_additive_dimensions),
            }
        )
    return rows


def _build_sql_agent_relational_model(
        sources: list[dict],
        max_sample_values: int = 10,
) -> dict:
    workbook_rows: dict[str, dict] = {}
    sheet_rows: list[dict] = []
    parent_report_rows: dict[str, dict] = {}
    ...
    for source in sources:
        ...
        if frame is None or frame.empty:
            continue
        internal_df = pd.DataFrame(frame).copy()
        file_name = str(source.get("file_name") or "uploaded_file")
        file_bytes = source.get("file_bytes") or b""
        content_sha256 = (
            hashlib.sha256(file_bytes).hexdigest()
            if file_bytes
            else hashlib.sha256(
                f"{file_name}|{internal_df.shape}|{list(internal_df.columns)}".encode("utf-8")
            ).hexdigest()
        )
        workbook_id = _stable_agent_id("wb", content_sha256)
        ...
        sheet_name = str(source.get("sheet_name") or "flat_table")
        sheet_index = int(source.get("sheet_index") or 0)
        sheet_id = _stable_agent_id("sh", workbook_id, sheet_index, sheet_name)
        parsing_mode = str(source.get("parsing_mode") or "auto")
        source_shape = source.get("source_shape") or internal_df.shape

        if "table_id" not in internal_df.columns:
            internal_df["table_id"] = "table_1"
        if "table_name" not in internal_df.columns:
            internal_df["table_name"] = sheet_name
        internal_df["table_id"] = internal_df["table_id"].map(lambda value: cell_text(value) or "table_1")
        internal_df["table_name"] = internal_df["table_name"].map(lambda value: cell_text(value) or sheet_name)

        def _semantic_table_name(row: pd.Series) -> str:
            base_name = cell_text(row.get("table_name", "")) or sheet_name
            column_group = cell_text(row.get("column_group", ""))
            column_group_ids = re.findall(r"\bt\d+\b", column_group.lower())
            if _looks_like_table_title_text(column_group) and len(column_group_ids) <= 1:
                return column_group
            metric = cell_text(row.get("metric", ""))
            metric_prefix = metric.split("|", 1)[0].strip() if "|" in metric else ""
            metric_prefix_ids = re.findall(r"\bt\d+\b", metric_prefix.lower())
            if _looks_like_table_title_text(metric_prefix) and len(metric_prefix_ids) <= 1:
                return metric_prefix
            return base_name

        internal_df["_agent_table_name"] = internal_df.apply(_semantic_table_name, axis=1)
        grouping_columns = ["table_id", "_agent_table_name"]
        grouped = list(internal_df.groupby(grouping_columns, dropna=False, sort=False))
        valid_parent_reports: set[str] = set()
        if "parent_report_name" in internal_df.columns:
            parent_child_pairs = (
                internal_df.assign(
                    _parent_report_text=internal_df["parent_report_name"].map(cell_text)
                )
                .loc[lambda frame: frame["_parent_report_text"].ne("")]
                .loc[:, ["_parent_report_text", "table_id", "_agent_table_name"]]
                .drop_duplicates()
            )
            if not parent_child_pairs.empty:
                parent_child_counts = parent_child_pairs.groupby(
                    "_parent_report_text",
                    sort=False,
                ).size()
                valid_parent_reports = set(
                    parent_child_counts[parent_child_counts >= 2].index.tolist()
                )
        sheet_observation_count = 0
        for group_index, (keys, group_df) in enumerate(grouped, start=1):
            table_id_value, table_name_value = keys if isinstance(keys, tuple) else (keys, sheet_name)
            source_range = ""
            if "source_range" in group_df.columns:
                source_ranges = [
                    cell_text(value)
                    for value in group_df["source_range"].tolist()
                    if cell_text(value)
                ]
                source_range = source_ranges[0] if source_ranges else ""
            ...
            logical_name = (
                ...
                f"{_safe_sql_name(sheet_name)}__"
                f"{_safe_sql_name(str(table_name_value))}__"
                f"{logical_table_id[-6:]}"
            )
            parent_report_name = ""
            if "parent_report_name" in group_df.columns:
                parent_values = [
                    cell_text(value)
                    for value in group_df["parent_report_name"].tolist()
                    if cell_text(value)
                ]
                if parent_values:
                    parent_report_name = pd.Series(parent_values).mode().iloc[0]
            if parent_report_name not in valid_parent_reports:
                parent_report_name = ""
            parent_report_id = (
                _stable_agent_id("report", sheet_id, parent_report_name)
                if parent_report_name
                else None
            )
            if parent_report_id:
                parent_row = parent_report_rows.setdefault(
                    parent_report_id,
                    {
                        "parent_report_id": parent_report_id,
                        "sheet_id": sheet_id,
                        "report_name": parent_report_name,
                        "description": (
                            f"Shared report or dashboard heading in worksheet '{sheet_name}' "
                            f"that groups multiple logical tables."
                        ),
                        "child_table_count": 0,
                        "search_terms_json": json.dumps(
                            [file_name, sheet_name, parent_report_name],
                            ensure_ascii=False,
                        ),
                    },
                )
                parent_row["child_table_count"] += 1

            public_group = prepare_sql_agent_table(group_df)
            if public_group.empty:
                continue
            value_text = (
                public_group["value"].map(_sql_text)
                if "value" in public_group.columns
                else pd.Series("", index=public_group.index, dtype="object")
            )
            value_numeric = (
                pd.to_numeric(public_group["value_numeric"], errors="coerce")
                if "value_numeric" in public_group.columns
                else pd.Series(np.nan, index=public_group.index, dtype="float64")
            )
            value_kind = (
                public_group["value_kind"].map(_sql_text)
                if "value_kind" in public_group.columns
                else pd.Series("text", index=public_group.index, dtype="object")
            )

            units = list(dict.fromkeys(
                cell_text(value)
                for value in public_group.get("unit", pd.Series(dtype="object")).tolist()
                if cell_text(value)
            ))
            sections = list(dict.fromkeys(
                cell_text(value)
                for value in public_group.get("section", pd.Series(dtype="object")).tolist()
                if cell_text(value)
            ))
            line_items = list(dict.fromkeys(
                cell_text(value)
                for value in public_group.get("line_item", pd.Series(dtype="object")).tolist()
                if cell_text(value)
            ))
            metrics = list(dict.fromkeys(
                cell_text(value)
                for value in public_group.get("metric", pd.Series(dtype="object")).tolist()
                if cell_text(value)
            ))
            table_type = _infer_agent_table_type(str(table_name_value), units)
            aggregation_policy = _agent_aggregation_policy(table_type, units)
            ...
            description = (
                f"{table_name_value} extracted from worksheet '{sheet_name}' in '{file_name}'. "
                f"Contains one row per line-item and metric observation."
            )
            ...
            logical_rows.append(
                {
                    ...
                    "display_name": str(table_name_value),
                    "table_type": table_type,
                    "description": description,
                    "grain": "One observed Excel value for one line item and one metric.",
                    "physical_table": "excel_observations",
                    "filter_sql": f"logical_table_id = '{logical_table_id}'",
                    "row_count": len(public_group),
                    "primary_measure": "value_numeric",
                    "default_aggregation": aggregation_policy["default_aggregation"],
                    "additivity": aggregation_policy["additivity"],
                    "non_additive_dimensions_json": json.dumps(aggregation_policy["non_additive_dimensions"]),
                    "requires_unit_filter": int(aggregation_policy["requires_unit_filter"]),
                    "known_units_json": json.dumps(units[:20], ensure_ascii=False),
                    "known_sections_json": json.dumps(sections[:30], ensure_ascii=False),
                    "search_terms_json": json.dumps(search_terms[:60], ensure_ascii=False),
                }
            )
            metric_rows.extend(
                [
                    {
                        "metric_id": _stable_agent_id("metric", logical_table_id, "observed_value"),
                        "logical_table_id": logical_table_id,
                        "metric_name": "observed_value",
                        "description": (
                            "Numeric observation from the workbook. Apply the table aggregation policy "
                            "and do not mix incompatible units."
                        ),
                        "expression_sql": "value_numeric",
                        "aggregation": aggregation_policy["default_aggregation"],
                        "unit_column": "unit",
                        "non_additive_dimensions_json": json.dumps(
                            aggregation_policy["non_additive_dimensions"]
                        ),
                        "requires_unit_filter": int(aggregation_policy["requires_unit_filter"]),
                    },
                    {
                        "metric_id": _stable_agent_id("metric", logical_table_id, "observation_count"),
                        "logical_table_id": logical_table_id,
                        "metric_name": "observation_count",
                        "description": "Count of extracted observations in the logical table.",
                        "expression_sql": "COUNT(*)",
                        "aggregation": "COUNT",
                        "unit_column": None,
                        "non_additive_dimensions_json": "[]",
                        "requires_unit_filter": 0,
                    },
                ]
            )

            profile_frame = pd.DataFrame(index=public_group.index)
            for dimension in SQL_AGENT_OBSERVATION_DIMENSIONS:
                if dimension in public_group.columns:
                    profile_frame[dimension] = public_group[dimension]
            profile_frame["value_text"] = value_text
            profile_frame["value_numeric"] = value_numeric
            profile_frame["value_kind"] = value_kind
            column_rows.extend(
                _build_agent_column_rows(
                    logical_table_id,
                    profile_frame,
                    aggregation_policy,
            ...
            original_group = group_df.reset_index(drop=True)
            public_group = public_group.reset_index(drop=True)
            value_text = value_text.reset_index(drop=True)
            value_numeric = value_numeric.reset_index(drop=True)
            value_kind = value_kind.reset_index(drop=True)
            fingerprint_counts: dict[str, int] = {}
            for row_position in range(len(public_group)):
                public_row = public_group.iloc[row_position]
                internal_row = original_group.iloc[min(row_position, len(original_group) - 1)]
                source_cell = _sql_text(internal_row.get("source_cell", ""))
                row_fingerprint = json.dumps(
                    {
                        dimension: _sql_text(public_row.get(dimension, ""))
                        for dimension in SQL_AGENT_OBSERVATION_DIMENSIONS
                    }
                    | {
                        "value_text": _sql_text(value_text.iloc[row_position]),
                        "value_numeric": (
                            None
                            if pd.isna(value_numeric.iloc[row_position])
                            else float(value_numeric.iloc[row_position])
                        ),
                        "value_kind": _sql_text(value_kind.iloc[row_position]),
                    },
                    ensure_ascii=False,
                    sort_keys=True,
                    default=str,
                )
                duplicate_ordinal = fingerprint_counts.get(row_fingerprint, 0)
                fingerprint_counts[row_fingerprint] = duplicate_ordinal + 1
                identity_parts = (
                    logical_table_id,
                    source_cell or row_fingerprint,
                    duplicate_ordinal,
                    _sql_text(public_row.get("line_item", "")),
                    _sql_text(public_row.get("metric", "")),
                    _sql_text(value_text.iloc[row_position]),
                )
                observation_id = _stable_agent_id("obs", *identity_parts, length=20)
                observation = {
                ...
                for dimension in SQL_AGENT_OBSERVATION_DIMENSIONS:
                    value = public_row.get(dimension, "")
                    if dimension in {"indent_level", "comparison_year"}:
                        parsed = pd.to_numeric(pd.Series([value]), errors="coerce").iloc[0]
                        observation[dimension] = None if pd.isna(parsed) else int(parsed)
                    else:
                        observation[dimension] = _sql_text(value)
                observation.update(
                    {
                        "value_text": _sql_text(value_text.iloc[row_position]),
                        "value_numeric": (
                        ...
                        "value_kind": _sql_text(value_kind.iloc[row_position]) or "text",
                    }
                )
                ...
                provenance_rows.append(
                    {
                        "observation_id": observation_id,
                        "source_range": _sql_text(internal_row.get("source_range", "")),
                        "source_cell": source_cell,
                        "source_row": _sqlite_value(internal_row.get("source_row", None)),
                        "source_column": _sql_text(internal_row.get("source_column", "")),
                        "extraction_method": _sql_text(internal_row.get("extraction_method", "")),
                    }
                )
            sheet_observation_count += len(public_group)
        ...
        sheet_rows.append(
            {
                ...
                "source_row_count": int(source_shape[0]) if source_shape else None,
                "source_column_count": int(source_shape[1]) if source_shape else None,
                "logical_table_count": len(grouped),
                "observation_count": sheet_observation_count,
            }
        )
    ...
    frames = {
        ...
        "agent_parent_reports": pd.DataFrame(parent_report_rows.values()),
        "agent_logical_tables": pd.DataFrame(logical_rows),
        "excel_observations": pd.DataFrame(observation_rows),
        "observation_provenance": pd.DataFrame(provenance_rows),
        "agent_columns": pd.DataFrame(column_rows),
        "agent_metrics": pd.DataFrame(metric_rows),
    }

    relationship_specs = [
        ("sheets_to_workbooks", "agent_sheets", "workbook_id", "agent_workbooks", "workbook_id", "many_to_one"),
        ("parent_reports_to_sheets", "agent_parent_reports", "sheet_id", "agent_sheets", "sheet_id", "many_to_one"),
        ("logical_tables_to_parent_reports", "agent_logical_tables", "parent_report_id", "agent_parent_reports", "parent_report_id", "many_to_one"),
        ("logical_tables_to_sheets", "agent_logical_tables", "sheet_id", "agent_sheets", "sheet_id", "many_to_one"),
        ("observations_to_logical_tables", "excel_observations", "logical_table_id", "agent_logical_tables", "logical_table_id", "many_to_one"),
        ("provenance_to_observations", "observation_provenance", "observation_id", "excel_observations", "observation_id", "one_to_one"),
        ("metrics_to_logical_tables", "agent_metrics", "logical_table_id", "agent_logical_tables", "logical_table_id", "many_to_one"),
    ]
    relationship_rows = [
        {
            "relationship_id": _stable_agent_id("rel", *spec),
            "relationship_name": spec[0],
            "from_table": spec[1],
            "from_column": spec[2],
            "to_table": spec[3],
            "to_column": spec[4],
    ...
    frames["agent_relationships"] = pd.DataFrame(relationship_rows)

    verified_query_rows: list[dict] = [
        {
            "question": "What logical tables are available?",
            "sql_query": (
                "SELECT logical_table_id, file_name, sheet_name, display_name, table_type, "
                "row_count, known_units_json FROM v_agent_table_catalog "
                "ORDER BY file_name, sheet_name, display_name LIMIT 100"
            ),
            "description": "Catalog discovery query that should run before selecting a business table.",
            "logical_table_id": None,
        },
        {
            "question": "Which columns and meanings are available for each logical table?",
            "sql_query": (
                "SELECT logical_name, column_name, sql_type, semantic_role, description, "
                "synonyms_json, sample_values_json FROM v_agent_column_catalog "
                "WHERE is_hidden = 0 ORDER BY logical_name, column_name LIMIT 200"
            ),
            "description": "Column discovery query with semantic roles, descriptions, synonyms, and examples.",
            "logical_table_id": None,
        },
        {
            "question": "Which metrics and aggregation rules are available?",
            "sql_query": (
                "SELECT logical_name, metric_name, description, expression_sql, aggregation, "
                "unit_column, non_additive_dimensions_json, requires_unit_filter "
                "FROM v_agent_metric_catalog ORDER BY logical_name, metric_name LIMIT 200"
            ),
            "description": "Metric discovery query with aggregation and non-additivity rules.",
            "logical_table_id": None,
        },
    ]
    for logical_row in logical_rows[:20]:
        logical_table_id = logical_row["logical_table_id"]
        display_name = logical_row["display_name"]
        verified_query_rows.append(
            {
                "question": f"Show sample observations from {display_name}.",
                "sql_query": (
                    "SELECT line_item, metric, unit, value_numeric, value_text "
                    "FROM v_excel_facts "
                    f"WHERE logical_table_id = '{logical_table_id}' "
                    "ORDER BY line_item, metric LIMIT 20"
                ),
                "description": "Safe detail query scoped to one logical table.",
                "logical_table_id": logical_table_id,
            }
        )
    ...
    catalog = {
        ...
        "purpose": "Natural-language SQL analysis of flattened, originally unstructured Excel and CSV files.",
        "physical_model": {
            "primary_fact_table": "excel_observations",
            "recommended_query_view": "v_excel_facts",
            "agent_visible_objects": [
                "v_agent_table_catalog",
                "v_agent_column_catalog",
                "v_agent_metric_catalog",
                "v_excel_facts",
                "agent_verified_queries",
            ],
            "agent_hidden_objects": ["observation_provenance"],
            "metadata_tables": [
                "agent_workbooks",
                "agent_sheets",
                "agent_parent_reports",
                "agent_logical_tables",
                "agent_columns",
                "agent_metrics",
                "agent_relationships",
                "agent_verified_queries",
            ],
            "private_provenance_table": "observation_provenance",
        },
        "agent_workflow": [
            "Query v_agent_table_catalog first and choose the logical_table_id relevant to the question.",
            "Query v_agent_column_catalog for that logical table before writing analytical SQL.",
            "Use v_excel_facts for analysis and always filter by logical_table_id unless the question explicitly spans tables.",
            "Use value_numeric for arithmetic and value_text for categorical or display-only observations.",
            "Filter or group by unit before aggregating and respect each table's additivity policy.",
            "Use verified SQL examples as patterns for ambiguous or recurring questions.",
            f"Limit detail queries to at most {SQL_AGENT_MAX_DETAIL_ROWS} rows unless the user explicitly requests more.",
        ],
        "safety": {
            "read_only": True,
            "recommended_connection_mode": "Open the SQLite database with mode=ro and enable PRAGMA query_only.",
            "allowed_statement_prefixes": ["SELECT", "WITH", "EXPLAIN"],
            ...
            "query_checker_required": True,
            "clarification_rules": [
                "Ask which workbook, sheet, or logical table when the question matches multiple candidates.",
                "Ask for a period when a time-sensitive question does not specify one and several periods exist.",
                "Ask for a unit or currency when aggregation would mix incompatible units.",
            ],
        },
        "parent_reports": [
            {
            ...
            for row in parent_report_rows.values()
        ],
        "logical_tables": [
            {
                **row,
                "known_units": json.loads(row["known_units_json"]),
                "known_sections": json.loads(row["known_sections_json"]),
                "search_terms": json.loads(row["search_terms_json"]),
                "non_additive_dimensions": json.loads(row["non_additive_dimensions_json"]),
            }
    ...
    return {
        "frames": frames,
        "verified_query_rows": verified_query_rows,
        "catalog": catalog,
        "ddl_sql": _sql_agent_ddl(),
    }


def build_sql_agent_package(
        sources: list[dict],
        max_sample_values: int = 10,
) -> dict:
    """Build a portable read-only SQLite database plus its semantic catalog."""
    model = _build_sql_agent_relational_model(
        sources,
        max_sample_values=max_sample_values,
    )
    connection = sqlite3.connect(":memory:")
    try:
        connection.executescript(model["ddl_sql"])
        insertion_order = [
            "agent_workbooks",
            "agent_sheets",
            "agent_parent_reports",
        ...
        for table_name in insertion_order:
            _insert_sqlite_rows(connection, table_name, model["frames"].get(table_name, pd.DataFrame()))

        verified_rows = []
        for row in model["verified_query_rows"]:
            valid, error = validate_sql_agent_query(row["sql_query"])
            status = "failed_validation"
            if valid:
                try:
                    connection.execute(row["sql_query"]).fetchmany(3)
                    status = "passed"
                except Exception as exc:
                    error = str(exc)
                    status = "failed_execution"
            verified_row = {
                "verified_query_id": _stable_agent_id("vq", row["question"], row["sql_query"]),
                "question": row["question"],
                "sql_query": row["sql_query"],
                "description": row["description"] + (f" Validation note: {error}" if error else ""),
                "logical_table_id": row.get("logical_table_id"),
                "validation_status": status,
            }
            verified_rows.append(verified_row)
        verified_frame = pd.DataFrame(verified_rows)
        _insert_sqlite_rows(connection, "agent_verified_queries", verified_frame)
        connection.commit()
        database_bytes = connection.serialize()
    finally:
        connection.close()

    model["frames"]["agent_verified_queries"] = verified_frame
    model["catalog"]["verified_queries"] = verified_rows
    model["catalog"]["statistics"] = {
        "workbook_count": int(len(model["frames"]["agent_workbooks"])),
        "sheet_count": int(len(model["frames"]["agent_sheets"])),
        "parent_report_count": int(len(model["frames"]["agent_parent_reports"])),
        "logical_table_count": int(len(model["frames"]["agent_logical_tables"])),
        "observation_count": int(len(model["frames"]["excel_observations"])),
        "verified_query_count": int((verified_frame["validation_status"] == "passed").sum()),
    }
    return {
        "database_bytes": database_bytes,
        "catalog": model["catalog"],
        "ddl_sql": model["ddl_sql"],
        "frames": model["frames"],
    }
...
def build_excel_schema_package(
    ...
    """Build schema metadata for Excel export without creating a database."""
    model = _build_sql_agent_relational_model(
        sources,
        max_sample_values=max_sample_values,
    )
    ...
    for row in model["verified_query_rows"]:
        valid, error = validate_sql_agent_query(row["sql_query"])
        schema_query_rows.append(
            {
                "verified_query_id": _stable_agent_id(
                    "vq",
                    row["question"],
                    row["sql_query"],
                ),
                ...
                "description": row["description"] + (
                    f" Validation note: {error}" if error else ""
                ),
                "logical_table_id": row.get("logical_table_id"),
                "validation_status": (
                    "syntax_checked" if valid else "failed_validation"
                ),
            }
        )
    verified_frame = pd.DataFrame(schema_query_rows)
    model["frames"]["agent_verified_queries"] = verified_frame
    model["catalog"]["verified_queries"] = schema_query_rows
    model["catalog"]["statistics"] = {
        "workbook_count": int(len(model["frames"]["agent_workbooks"])),
        "sheet_count": int(len(model["frames"]["agent_sheets"])),
        "parent_report_count": int(len(model["frames"]["agent_parent_reports"])),
        "logical_table_count": int(len(model["frames"]["agent_logical_tables"])),
        "observation_count": int(len(model["frames"]["excel_observations"])),
        "verified_query_count": int(
            (verified_frame.get("validation_status") == "syntax_checked").sum()
            if not verified_frame.empty
            else 0
        ),
    }
    return {
...
def execute_sql_agent_query(database_bytes: bytes, query: str, max_rows: int = SQL_AGENT_MAX_DETAIL_ROWS) -> dict:
    """Execute one validated read-only query against an exported SQL-agent database."""
    valid, error = validate_sql_agent_query(query)
    if not valid:
        return {"ok": False, "error": error, "columns": [], "rows": []}
    connection = sqlite3.connect(":memory:")
    try:
        ...
        rows = cursor.fetchmany(max(int(max_rows), 1) + 1)
...
def extract_and_flatten_sheet(file_bytes: bytes, sheet_name: str, preferred_profile: str = "auto") -> pd.DataFrame:
    ...
    flat_detection = infer_flat_table_structure(quick_df)
    ...
    auto_profile, _, _, _ = infer_report_layout_from_quick_df(quick_df)
...
def _extract_like_one_sheet_mode(file_bytes: bytes, sheet_name: str) -> pd.DataFrame:
    ...
    if not has_report_signals and not has_matrix_signals:
        result = _read_flat_excel_sheet(file_bytes, sheet_name, header_row=0)
        if result is not None and not result.empty and "tab_name" not in result.columns:
            result.insert(0, "tab_name", sheet_name)
        return result
...
def to_multisheet_excel_bytes(
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        for sheet_name, df in frames.items():
            ...
            df.to_excel(writer, sheet_name=_safe_sheet_name(requested_name), index=False)
        if schema_df is not None and not schema_df.empty:
            schema_df.to_excel(writer, sheet_name="Schema", index=False)
            _format_schema_excel_sheet(
                writer.sheets.get("Schema"),
...
def render_cleanup_and_preview(
        df: pd.DataFrame,
        file_stem: str,
        key_prefix: str,
        sql_agent_context: dict | None = None,
):
    ...
    with st.expander("Edit the table with pandas-style operations", expanded=False):
        ...
        with col_dash:
            dash_split_mode = st.selectbox(
                ...
                help="Leave off for compact SQL-agent output; use the explicit split control when needed.",
            )
        dash_split_mode_map = {
            "Off": "off",
            "Spaced only ( - )": "spaced",
    ...
    cleanup_cache_version = "v3_sql_agent_compact"
    cleanup_sig = _hl.md5(
        str((
    ...
    context = dict(sql_agent_context or {})
    context["frame"] = final_df
    context.setdefault("file_name", f"{file_stem}.xlsx")
    context.setdefault("sheet_name", "flat_table")
    context.setdefault("sheet_index", 0)
    context.setdefault("parsing_mode", "flattened")
    context.setdefault("source_shape", df.shape)
    context_bytes = context.get("file_bytes") or b""
    package_sig = hashlib.sha256(
        (
            cleanup_sig
            + str(context.get("file_name", ""))
            + str(context.get("sheet_name", ""))
            + hashlib.sha256(context_bytes).hexdigest()
        ).encode("utf-8")
    ).hexdigest()[:16]
    package_state_key = f"{kp}_schema_package_{package_sig}"
    dl_key = f"{kp}_download"
    dl_state_key = f"{kp}_excel_bytes_{cleanup_sig}_{package_sig}"

    def _prepare_download():
        package = st.session_state.get(package_state_key)
        if package is None:
            package = build_excel_schema_package([context])
            st.session_state[package_state_key] = package
        schema_df = build_embedded_schema_frame(package)
        st.session_state[dl_state_key] = to_excel_bytes(
            final_df,
            schema_df=schema_df,
        )
    ...
    if dl_state_key not in st.session_state:
        ...
        st.download_button(
            "Download Common Excel Workbook",
            data=st.session_state[dl_state_key],
            file_name=f"{file_stem}_common_flat_file.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True,
            key=dl_key,
        )
...
def _extract_sql_agent_sources_from_uploads(uploaded_files) -> tuple[list[dict], list[str]]:
    """Flatten every uploaded file and worksheet into SQL-agent source records."""
    sources: list[dict] = []
    errors: list[str] = []
    for uploaded_file in uploaded_files:
        ...
        if suffix == ".csv":
            try:
                frame = _read_csv_bytes(file_bytes)
                sources.append(
                    {
                        "file_name": file_name,
                        "file_bytes": file_bytes,
                        "sheet_name": "flat_table",
                        "sheet_index": 0,
                        "parsing_mode": "flat_csv",
        ...
        try:
            ...
            errors.append(f"{file_name}: could not open workbook ({exc})")
            continue
        ...
        for sheet_index, sheet_name in enumerate(excel_file.sheet_names):
            try:
                quick_df = _quick_read_cached(file_bytes, sheet_name)
                flat_detection = infer_flat_table_structure(quick_df)
                frame = _extract_like_one_sheet_mode(file_bytes, sheet_name)
                if frame is None or frame.empty:
                    frame = pd.DataFrame(
                    ...
                    sources.append(
                        {
                            ...
                            "sheet_name": sheet_name,
                            "sheet_index": sheet_index,
                            "parsing_mode": "flat_fallback",
                            "source_shape": fallback.shape,
                            "frame": fallback,
                        }
                    )
                except Exception as fallback_exc:
                    errors.append(
                        f"{file_name} / {sheet_name}: {exc}; fallback failed ({fallback_exc})"
                    )
...
def render_unified_file_tab():
    ...
    if len(uploaded_files) > 1:
        multi_file_mode = st.radio(
            "Multiple-file mode",
            [
                "Build one common Excel workbook from all uploaded files",
                "Inspect and process one file",
            ],
            index=0,
            key="unified_multiple_file_mode",
        )
        if multi_file_mode == "Build one common Excel workbook from all uploaded files":
            file_hashes = [
                f"{uploaded.name}:{hashlib.sha256(uploaded.getvalue()).hexdigest()}"
                for uploaded in uploaded_files
            ]
            workspace_sig = hashlib.sha256("|".join(file_hashes).encode("utf-8")).hexdigest()[:16]
            workspace_state_key = f"_multi_file_common_workbook_{workspace_sig}"
            st.caption(
                "All CSV files and all worksheets in every Excel workbook will be flattened, "
                "kept as separate data tabs, and documented in one embedded Schema tab."
            )
            if workspace_state_key not in st.session_state:
                if st.button(
                    "Build Common Excel Workbook",
                    use_container_width=True,
                    key="unified_build_multi_file_workbook",
                ):
                    with st.spinner("Extracting all files and building the common workbook…"):
                        sources, errors = _extract_sql_agent_sources_from_uploads(uploaded_files)
                        package = build_excel_schema_package(sources)
                        schema_df = build_embedded_schema_frame(package)
                        export_frames = {
                            (
                                f"{Path(source['file_name']).stem}__"
                                f"{source.get('sheet_name', 'flat_table')}"
                            ): prepare_sql_agent_table(source["frame"])
                            for source in sources
                            if source.get("frame") is not None
                            and not source["frame"].empty
                        }
                        st.session_state[workspace_state_key] = {
                            "package": package,
                            "errors": errors,
                            "excel_bytes": to_multisheet_excel_bytes(
                                export_frames,
                                schema_df=schema_df,
                            ),
                        }

            workspace_result = st.session_state.get(workspace_state_key)
            if workspace_result is None:
                st.info("Click 'Build Common Excel Workbook' to process all uploaded files.")
                return
            package = workspace_result["package"]
            errors = workspace_result["errors"]
            if errors:
                with st.expander(f"{len(errors)} file or sheet warning(s)", expanded=False):
                    for error in errors:
                        st.warning(error)

            statistics = package["catalog"].get("statistics", {})
            metric_cols = st.columns(4)
            metric_cols[0].metric("Workbooks", f"{statistics.get('workbook_count', 0):,}")
            metric_cols[1].metric("Sheets", f"{statistics.get('sheet_count', 0):,}")
            metric_cols[2].metric("Logical tables", f"{statistics.get('logical_table_count', 0):,}")
            metric_cols[3].metric("Observations", f"{statistics.get('observation_count', 0):,}")

            logical_preview = package["frames"].get("agent_logical_tables", pd.DataFrame())
            if not logical_preview.empty:
                st.markdown("### Logical Table Catalog")
                preview_columns = [
                    column
                    for column in (
                        "logical_table_id",
                    ...
                    if column in logical_preview.columns
                ]
                st.dataframe(logical_preview[preview_columns].head(200), use_container_width=True)

            st.download_button(
                "Download Common Excel Workbook",
                data=workspace_result["excel_bytes"],
                file_name="uploaded_files_common_flat_file.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True,
                key="unified_multi_file_excel_schema_download",
            )
            return
    ...
    if suffix == ".csv":
        ...
        df: pd.DataFrame = _read_csv_bytes(csv_bytes)
    ...
    if len(xls.sheet_names) > 1:
        ...
        if workbook_mode == "Process selected sheets separately (recommended)":
            ...
            for sname in sheets_to_include:
                package_frame = finalize_extracted_sheet(
                    split_outputs[sname].copy(),
                    strip_text=strip_text,
                    split_hierarchy=False,
                )
                batch_frames[sname] = package_frame
                package_sources.append(
                    {
                        "file_name": uploaded_file.name,
                        "file_bytes": file_bytes,
                        "sheet_name": sname,
                        "sheet_index": xls.sheet_names.index(sname),
                        "parsing_mode": "auto_excel",
                        "source_shape": _quick_read_cached(file_bytes, sname).shape,
                        "frame": package_frame,
                    }
                )
            sql_package_key = f"_multi_sheet_schema_package_{batch_sig}"

            def _prepare_batch_download():
                workbook_package = st.session_state.get(sql_package_key)
                if workbook_package is None:
                    workbook_package = build_excel_schema_package(package_sources)
                    st.session_state[sql_package_key] = workbook_package
                schema_df = build_embedded_schema_frame(workbook_package)
                st.session_state[batch_cache_key] = to_multisheet_excel_bytes(
                    batch_frames,
                    schema_df=schema_df,
                )
            ...
            if batch_cache_key not in st.session_state:
                st.button(
                    "Prepare Common Excel Workbook",
                    on_click=_prepare_batch_download,
                    use_container_width=True,
                    key="unified_separate_prepare_download",
                )
            else:
                st.download_button(
                    "Download Common Excel Workbook",
                    data=st.session_state[batch_cache_key],
                    file_name=f"{Path(uploaded_file.name).stem}_common_flat_file.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True,
    ...
    if flat_detection["is_flat"]:
        ...
        reason = (
            f"{flat_detection['reason']} "
            "Skipping report-block extraction."
        )
    else:
        suggested_mode_default = "report"
        if has_report_signals or has_matrix_signals:
            reason = f"{detection_reason} Using {suggested_layout} extraction profile."
        else:
            reason = (
                f"{flat_detection['reason']} Structure is ambiguous, so report "
                f"extraction remains enabled with the {suggested_layout} profile."
            )
    ...
    if active_mode == "flat":
        flat_df = _read_flat_excel_sheet(
            file_bytes,
            sheet_name,
            header_row=(
                flat_detection["header_row"]
                if flat_detection["is_flat"]
                else 0
            ),
        )
    ...
    if extracted_df.empty:
        ...
        render_cleanup_and_preview(
            ...
            sql_agent_context={
                ...
                "sheet_index": xls.sheet_names.index(sheet_name),
                "parsing_mode": "flat_fallback",
                "source_shape": raw_df.shape,
            },
        )
        return

    public_extracted_df = prepare_sql_agent_table(extracted_df)

    st.markdown("### Extracted Tables Preview")
    col_rows, col_cols, col_blocks = st.columns(3)
    col_rows.metric("Rows", f"{len(public_extracted_df):,}")
    col_cols.metric("Columns", f"{len(public_extracted_df.columns):,}")
    col_blocks.metric(
        "Tables",
        (
            f"{public_extracted_df['table_id'].nunique():,}"
            if "table_id" in public_extracted_df.columns
            else (
                f"{public_extracted_df['table_name'].nunique():,}"
                if "table_name" in public_extracted_df.columns
                else "n/a"
            )
        ),
    )
    st.dataframe(public_extracted_df.head(200), use_container_width=True)
    ...
    id_cols = [
        ...
        if col in public_extracted_df.columns
    ]
    if id_cols:
        mapping_df = public_extracted_df[id_cols].drop_duplicates().sort_values(id_cols).reset_index(drop=True)
        st.markdown("### Logical Tables")
    ...
    render_cleanup_and_preview(
        ...
        sql_agent_context={
            ...
            "sheet_name": sheet_name,
            "sheet_index": xls.sheet_names.index(sheet_name),
            "parsing_mode": extraction_profile,
            "source_shape": raw_df.shape,
        },
    )
...
def generate_structural_schema(
    ...
    """Compatibility wrapper returning the v4 relational SQL-agent catalog."""
    ...
    package = build_sql_agent_package(
        [context],
