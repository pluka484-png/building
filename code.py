...
def _lightweight_blank_mask(raw_df: pd.DataFrame) -> np.ndarray:
    ...
    return blank_flat.reshape(values.shape)
...
def _candidate_data_rows_for_region(raw_df: pd.DataFrame) -> list[tuple[int, int]]:
    ...
    for row_idx, row in enumerate(values):
        ...
        # A period-header row such as "in % | 2021 | 2022 | 2023" is not data.
...
def auto_flatten_rectangular_table(raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for row_idx in range(lookback_start, data_start):
        ...
        if (
            ...
            and (payload_non_blank == 0 or repeated_title)
        ):
    ...
    header_rows = [
        row_idx
        for row_idx in range(max(0, title_row + 1), data_start)
    ...
    for row_idx in data_rows:
        ...
        for col_idx in value_cols:
            ...
            record = {
                ...
                "unit": unit,
                "parent_line_item": parent_line_item,
                "line_item_path": line_item_path,
                "line_item": line_item,
                "metric": metric,
                "value": value,
...
def auto_flatten_report_tables(raw_df: pd.DataFrame, extraction_profile: str = "auto") -> pd.DataFrame:
    ...
    if extracted_regions:
        ...
        source_keys = [
            col
            for col in ("table_id", "source_cell", "metric", "line_item")
...
def render_unified_file_tab():
    ...
    col_blocks.metric(
        "Tables",
        (
            f"{extracted_df['table_id'].nunique():,}"
            if "table_id" in extracted_df.columns
            else (
                f"{extracted_df['block_id'].nunique():,}"
                if "block_id" in extracted_df.columns
                else "n/a"
            )
        ),
    )
