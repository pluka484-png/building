...
def _refresh_flat_file_schema_artifacts(files_db: FilesDatabaseManager) -> None:
    """Synchronize agent schema context and the downloadable common workbook."""
    if files_db is None or not files_db.tables_info:
        return

    # Older app versions loaded Schema as a regular SQLite table. Remove that
    # stale table so the AI sees only business data tables.
    for stale_name in [
        name for name in list(files_db.tables_info)
        if str(name).lower() == "schema"
    ]:
        try:
            if files_db.connection is not None:
                files_db.connection.exec_driver_sql(
                    f'DROP TABLE IF EXISTS "{stale_name}"'
                )
        except Exception:
            pass
    ...
    files_db.schema_router_index = _build_schema_router_index(
        files_db,
        package,
        workbook_frames,
    )
...
def _schema_terms(value: Any) -> set[str]:
    ...
    stopwords = {
        ...
        "from", "has", "have", "how", "in", "is", "it", "of", "on",
        "or", "show", "the", "this", "to", "was", "what", "which", "with",
    }
    tokens = {
        token
...
def _infer_router_column_role(column: str, dtype: str) -> str:
    normalized = column.lower()
    if normalized in {
        "value_numeric", "amount", "balance", "revenue", "cost", "quantity", "count",
    } or any(
        token in normalized
        for token in ("amount", "value", "balance", "revenue", "cost")
    ):
        return "measure"
    if any(
        token in normalized
        for token in ("date", "period", "year", "quarter", "month")
    ):
        return "time_dimension"
    if normalized in {
        "line_item", "line_item_path", "parent_line_item", "section",
    }:
...
def _build_schema_router_index(
    files_db: FilesDatabaseManager,
    schema_package: dict,
    workbook_frames: dict[str, pd.DataFrame],
) -> list[dict]:
    ...
    for table_name, info in files_db.tables_info.items():
        ...
        column_types = {
            ...
            for column, dtype in info.get("column_types", {}).items()
        }
        source_sheet = str(info.get("source_sheet") or table_name)
        ...
        for logical in related_logical:
            semantic_terms.update(_schema_terms(logical.get("display_name", "")))
            semantic_terms.update(_schema_terms(logical.get("description", "")))
            semantic_terms.update(
                _schema_terms(" ".join(logical.get("search_terms", [])))
            )
        ...
        for column in columns:
            ...
            if column in frame.columns:
                for value in frame[column].dropna().tolist():
                    ...
                    if rendered and rendered.lower() not in {"nan", "none"}:
                        sample_values.append(rendered[:80])
                    if len(dict.fromkeys(sample_values)) >= 5:
                        break
            ...
            column_records.append(
                {
                    ...
                    "role": _infer_router_column_role(
                        column,
                        column_types.get(column, ""),
                    ),
                    "samples": sample_values,
                    "name_terms": sorted(_schema_terms(column)),
                    "sample_terms": sorted(
                        _schema_terms(" ".join(sample_values))
                    ),
                }
            )
        ...
        index.append(
            {
                ...
                "terms": sorted(
                    _schema_terms(table_name)
                    | _schema_terms(source_sheet)
                    | _schema_terms(info.get("source_file", ""))
                    | semantic_terms
                ),
            }
        )
...
def _route_schema_for_question(
    files_db: FilesDatabaseManager,
    user_question: str,
    max_tables: int = 3,
    max_columns: int = 14,
    ...
    question_terms = _schema_terms(user_question)
    question_text = re.sub(
        r"[^a-z0-9]+", " ", str(user_question or "").lower()
    ).strip()
    ranked = []
    for table in index:
        ...
        for column in table.get("columns", []):
            column_score = (
                5.0
                * len(question_terms & set(column.get("name_terms", [])))
                + 1.5
                * len(question_terms & set(column.get("sample_terms", [])))
            )
            normalized_column = re.sub(
                r"[^a-z0-9]+", " ", str(column.get("name", "")).lower()
            ).strip()
            if normalized_column and normalized_column in question_text:
                column_score += 10.0
            if column.get("role") == "measure" and question_terms & {
                "sum", "total", "average", "avg", "amount", "value",
            }:
                column_score += 1.5
        ...
        ranked.append(
            {
                ...
                "columns": [
                    column
                    for column_score, column in column_scores
                    if column_score > 0
                ][:max_columns],
            }
        )
    ...
    ranked.sort(
        key=lambda item: (
            -item["score"], str(item["table"].get("table_name", ""))
        )
    )
    ...
    for item in selected:
        if not item["columns"]:
            ...
            continue
        selected_names = {
            str(column.get("name", "")) for column in item["columns"]
        }
        support_roles = {
            "measure",
            "time_dimension",
    ...
    if len(selected) >= 2:
        ...
        ambiguous = (
            top_score <= 0
            or second_score >= max(top_score * 0.80, top_score - 3.0)
        )
    return {
        "selected": selected,
        "ambiguous": ambiguous,
...
def _format_routed_schema_context(route: dict) -> str:
    ...
    for item in selected:
        ...
        if table.get("table_types"):
            lines.append(
                "  Semantic type: " + ", ".join(table["table_types"])
            )
        if table.get("aggregation_rules"):
            lines.append(
                "  Aggregation policy: "
                + ", ".join(table["aggregation_rules"])
            )
    ...
    if route.get("ambiguous"):
        candidate_names = [
            item["table"]["table_name"] for item in selected
        ]
        lines.append(
...
def _attach_schema_context_to_agent(
    sql_agent: FilesSQLAgent,
    files_db: FilesDatabaseManager,
) -> None:
    """Attach the router index without sending the full schema."""
    try:
        sql_agent.schema_router_index = list(
            getattr(files_db, "schema_router_index", []) or []
        )
    except Exception:
        # The query wrapper still routes schema per request even when the
        # external agent class disallows custom attributes.
        pass


def _question_with_schema_context(user_question: str) -> str:
    """Route and inject only schema relevant to one AI request."""
    files_db = st.session_state.files_db
    if files_db is None:
        return user_question
    route = _route_schema_for_question(files_db, user_question)
    if not route.get("selected"):
        return user_question
    schema_context = _format_routed_schema_context(route)
    return (
        f"{schema_context}\n\n"
        "USER QUESTION:\n"
        f"{user_question}\n\n"
        "Answer the user question. Do not mention this injected schema contract."
    )
...
def load_data(
    ...
    try:
        with st.spinner("Loading files..."):
            ...
            if loaded_count > 0:
                try:
                    _refresh_flat_file_schema_artifacts(files_db)
                except Exception as exc:
                    st.warning(
...
def load_uploaded_files(uploaded_files, selected_sheets=None, flat_file_overrides=None):
    ...
    if uploaded_files:
        ...
        _attach_schema_context_to_agent(sql_agent, files_db)
                        ...
                        if final_df is not None and not final_df.empty:
                            ...
                            with apply_col:
                                if st.button(
                                    ...
                                    if replaced_table:
                                        _refresh_flat_file_schema_artifacts(
                                            st.session_state.files_db
                                        )
...
def execute_query(user_question: str):
    ...
    with st.spinner("Generating SQL and executing query..."):
        contextual_question = _question_with_schema_context(user_question)
        result = st.session_state.sql_agent.execute_query_with_explanation(
            contextual_question
        )
