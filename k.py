...
# Extractor bookkeeping columns that group rows into blocks/schemas. They are
# needed while extracting but are stripped from every public output.
_BLOCK_BOOKKEEPING_COLUMNS = (
    "block_id",
    "schema_group_id",
    "source_block_id",
...
def _drop_block_bookkeeping_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Remove the extractor's block/schema bookkeeping columns from an output frame."""
    return df.drop(
        columns=[col for col in _BLOCK_BOOKKEEPING_COLUMNS if col in df.columns],
        errors="ignore",
    )
...
def _looks_like_date_or_period_token(token: str) -> bool:
    if not token:
        return False
    if re.fullmatch(r"(?:19|20)\d{2}\s*(?:a|n|fc|f|budget|plan)", token, flags=re.IGNORECASE):
        return True
    ...
    if re.fullmatch(r"(?:0?[1-9]|1[0-2])M\s*\d{2,4}", token, flags=re.IGNORECASE):
        return True
    if re.fullmatch(r"\d{2,4}\s*(?:0?[1-9]|1[0-2])M", token, flags=re.IGNORECASE):
        return True
    return False
...
def _candidate_merged_titles(merged_ranges: list) -> list[tuple[dict, str]]:
    """Merged ranges that look like report titles, as (range, title) pairs.

    Several merged ranges sharing one row are column-group headers (segment /
    region / scenario bands over value columns), not report titles — a real
    parent title is the only merged range on its row.
    """
    ...
    for merged in merged_ranges:
        ...
        if merged_rows_seen.get(int(merged.get("min_row", 0)), 0) >= 2:
            continue
        candidates.append((merged, title))
    return candidates
...
def _parent_report_titles_for_regions(
    ...
    for merged, title in _candidate_merged_titles(merged_ranges):
...
def propagate_section_context(raw_df: pd.DataFrame, section_col: int = 0) -> pd.DataFrame:
    ...
    first_text_like = has_any & ~context.numeric_mask[row_idx, safe_first_col] & ~context.unit_mask[
        row_idx, safe_first_col]
...
def _row_label_and_numeric_hits(values, row_idx: int, data_col_start: int,
                                col_count: int) -> tuple[int, int]:
    """Count non-blank label cells left of the data band and numeric cells inside it.

    A row with labels on the left and several numbers on the right looks like a
    data row rather than a header row.
    """
    label_hits = sum(
        1
    ...
    return label_hits, numeric_hits
...
def _detect_summary_header_band(raw_df: pd.DataFrame, data_start: int, data_col_start: int) -> list[int]:
    ...
    for row_idx in range(min(data_start, len(raw_df))):
        ...
        saw_non_blank = True
        label_hits, numeric_hits = _row_label_and_numeric_hits(
            values, row_idx, data_col_start, col_count
        )
...
def _looks_like_financial_summary_layout(raw_df: pd.DataFrame) -> bool:
    ...
    for row_idx in range(data_start, sample_end):
        ...
        numeric_hits = sum(1 for col_idx in range(data_col_start, raw_df.shape[1]) if
                           _parse_numeric_value(row.iloc[col_idx]) is not None)
...
def _is_col0_unit_anchor(value) -> bool:
    ...
    return text in {"eur", "eur mn", "eur bn", "usd", "usd mn", "usd bn", "gbp", "gbp mn", "gbp bn"} or text.startswith(
        ("eur ", "usd ", "gbp ", "in %"))
...
def auto_flatten_horizontal_balance_sheet(raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for start_col in block_starts:
        ...
        segment_name = cell_text(raw_df.iloc[3, start_col + 2]) if len(raw_df) > 3 and start_col + 2 < raw_df.shape[
            1] else ""
        ...
        for row_idx in range(5, len(raw_df)):
            ...
            # Shared context for every value emitted from this row; metric and
            # value are spliced in between so column order stays stable.
            row_context = {
            ...
            def _emit(metric_name, cell_value):
                record = {
                    **row_context,
                    "metric": metric_name,
                    "value": cell_value,
                ...
                record.update(metric_context(metric_name))
                records.append(record)
            ...
            if delta_col < raw_df.shape[1]:
                ...
                if not is_blank(delta_value):
                    row_has_value = True
                    _emit("Delta 31.12.25 / 31.12.24", delta_value)
...
def _fallback_matrix_flatten(raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _looks_like_period_token(token: str) -> bool:
        ...
        if len(text) > 35:
            return False
        if RE_HEADER_PERIOD_QUALIFIER.fullmatch(text):
            return True
        if re.fullmatch(r"(?:19|20)\d{2}\s*(?:a|n|fc|f|budget|plan)", text):
            return True
    ...
    # Pick a broad section/title, preferring standalone section rows near data start.
    # This avoids consuming dense top header bands as the section value.
    section = ""
    for row_idx in reversed(header_rows):
        candidate = section_name_validated(
            raw_df.iloc[row_idx],
            expected_col=section_col,
            allow_hybrid=False,
        )
        if candidate:
            section = candidate
    ...
    if not section:
        for row_idx in range(min(8, row_count)):
            row_tokens = [
                _get_cached_cell_text(row_idx, col_idx)
                for col_idx in range(min(col_count, max(metric_col + 1, 10)))
            ]
            dense_text_tokens = [
                token
                for token in row_tokens
                if token and token.upper() not in {"#REF!", "#DIV/0!"} and not _looks_like_period_token(token)
            ]
            if len(dense_text_tokens) >= 3:
                continue
...
def _row_declares_column_headers(context: SheetScanContext, row_idx: int) -> bool:
    """True when a row labels value columns instead of carrying observations.

    This is what tells a genuinely new stacked table — which repeats its own
    header band below the gap — apart from a sparse stretch inside one wide
    matrix, which keeps reusing the header band above it. Period-like tokens
    (`2024`, `Q1 2025`, `31.12.2025`) are headers even when Excel stored them
    as numbers, so they are excluded from the numeric-payload veto.
    """
    payload_tokens = [
        str(context.text[row_idx, col_idx]).strip()
        for col_idx in range(context.text.shape[1])
        if bool(context.data_like_mask[row_idx, col_idx])
    ]
    if any(not _looks_like_date_or_period_text(token) for token in payload_tokens):
        return False
    ...
    if len(tokens) < 2:
        return False
    period_tokens = sum(
        1 for token in tokens if _looks_like_date_or_period_text(token)
    )
    return period_tokens >= 2
...
def _split_table_region_vertically(
    ...
    for previous, current in zip(clusters, clusters[1:]):
        ...
        # Without an explicit ruled title, split only on a meaningful whitespace
        # gap. Sparse non-candidate stretches inside one wide matrix should stay
        # in the same region so they can reuse the same header band.
        if len(blank_rows) >= 2:
            split_starts.append(blank_rows[-1] + 1)
            continue

        # A single blank row is enough when the rows below it re-declare the
        # column headers: a table that continues under a shared header band
        # never repeats it, so a fresh header means a fresh table.
        if blank_rows and any(
                _row_declares_column_headers(context, row_idx)
                for row_idx in range(blank_rows[-1] + 1, current_start)
        ):
            split_starts.append(blank_rows[-1] + 1)
...
def _composite_headers_for_region(
    ...
    for row_idx in header_rows:
        ...
        non_blank_cells = [
            (col_idx, str(context.text[row_idx, col_idx]).strip())
            for col_idx in range(raw_df.shape[1])
            if str(context.text[row_idx, col_idx]).strip()
        ]
        # A single left-side label row is typically a section divider, not a
        # value-column header (e.g., "Own Book"). Keep it out of metric headers.
        if len(non_blank_cells) == 1:
            only_col, only_token = non_blank_cells[0]
            if only_col < first_value_col and not _looks_like_date_or_period_text(only_token):
        ...
        if len(set(value_tokens)) == 1 and value_tokens:
            ...
            if _looks_like_date_or_period_text(sole_token) or sole_token.lower() in repeated_header_tokens:
                for col_idx in value_cols:
        ...
        current = ""
        # Start from col 0 so left-anchored merged labels (common in matrix headers)
        # can propagate into value columns even when value-area cells are blank.
        for col_idx in range(raw_df.shape[1]):
            token = str(context.text[row_idx, col_idx]).strip()
            if token:
                current = token
    ...
    def _period_like_header(token: str) -> bool:
        text = str(token or "").strip()
        if not text:
            return False
        if RE_HEADER_PERIOD_QUALIFIER.fullmatch(text):
            return True
        return _looks_like_date_or_period_text(text)
    ...
    for col_idx in value_cols:
        ...
        # If header extraction is empty/noisy, recover nearest period token
        # from the raw header band for this value column.
        if not parts:
            recovered = ""
            for row_idx in reversed(header_rows):
                direct = str(context.text[row_idx, col_idx]).strip()
                if _period_like_header(direct):
                    recovered = direct
                    break
                for left_col in range(col_idx - 1, -1, -1):
                    candidate = str(context.text[row_idx, left_col]).strip()
                    if not candidate:
                        continue
                    if _period_like_header(candidate):
                        recovered = candidate
                    break
                if recovered:
                    break
    ...
    for col_idx, header in base_headers.items():
        if duplicate_counts.get(header, 0) > 1 and header.lower().startswith("column "):
...
def auto_flatten_rectangular_table(raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _is_section_only_header_row(row_idx: int) -> bool:
        non_blank = int(context.non_blank_count[row_idx])
        if non_blank != 1:
            return False
        first_col = int(context.first_non_blank_col[row_idx])
        if first_col < 0 or first_col >= data_col_start:
            return False
        token = str(context.text[row_idx, first_col]).strip()
        return bool(token) and not _looks_like_date_or_period_text(token)

    header_rows = [row_idx for row_idx in header_rows if not _is_section_only_header_row(row_idx)]
    if not header_rows and data_start > 0:
        ...
        header_rows = [
            ...
            if int(context.non_blank_count[row_idx]) > 0
               and bool((~context.blank_mask[row_idx, data_col_start:]).any())
               and not _is_section_only_header_row(row_idx)
        ]
    if not header_rows and data_start > 0:
    ...
    records: list[dict] = []
    current_section = ""
    left_label_carry = ["" for _ in range(max(data_col_start, 0))]
    top_rules = raw_df.attrs.get("excel_row_top_rule_flags") or []
    bottom_rules = raw_df.attrs.get("excel_row_bottom_rule_flags") or []
    ...
    for row_idx in data_rows:
        ...
        explicit_cols = [col_idx for col_idx, token in enumerate(raw_left_tokens) if token]
        if not explicit_cols:
            continue

        deepest_explicit_col = max(explicit_cols)
        leaf_token = raw_left_tokens[deepest_explicit_col].strip().lower()
        row_has_top_rule = bool(top_rules[row_idx]) if row_idx < len(top_rules) else False
        prev_has_bottom_rule = bool(bottom_rules[row_idx - 1]) if 0 < row_idx <= len(bottom_rules) else False
        suppress_parent_carry = leaf_token in {"total", "grand total", "subtotal"} and (
            row_has_top_rule or prev_has_bottom_rule
        )

        label_tokens: list[str] = []
        for col_idx in range(deepest_explicit_col + 1):
            token = raw_left_tokens[col_idx]
            if token:
                left_label_carry[col_idx] = token
            elif (
                col_idx < deepest_explicit_col
                and left_label_carry[col_idx]
                and not suppress_parent_carry
            ):
                # Fill ancestor slots only; never synthesize the row leaf label.
                token = left_label_carry[col_idx]
            if token:
...
def _auto_flatten_single_region_legacy(raw_df: pd.DataFrame, extraction_profile: str = "auto") -> pd.DataFrame:
    ...
    strategy_order = []
    if normalized.empty and (
            profile == "matrix" or (profile == "auto" and has_matrix_signals and not looks_like_rates_sheet)):
        strategy_order = [
    ...
    normalized = _drop_block_bookkeeping_columns(normalized)
...
def _cluster_columns_lack_own_header(
        raw_df: pd.DataFrame | None,
        columns: list[int],
        first_data_row: int,
) -> bool:
    """True when a column cluster never labels itself on the worksheet.

    A stray number parked beside a report inherits whatever header the real
    value block declared, so the extracted metric text cannot give it away.
    The sheet can: a reported column labels itself somewhere above its table's
    first observation, and a parked helper cell never does. Coordinates are the
    extractor's 1-based Excel row and column indexes.
    """
    if raw_df is None or raw_df.empty:
        return False
    header_zone = min(int(first_data_row) - 1, raw_df.shape[0])
    if header_zone <= 0:
        return False
    ...
    for column in columns:
        ...
        if col_idx < 0 or col_idx >= raw_df.shape[1]:
            continue
        if bool((~context.blank_mask[:header_zone, col_idx]).any()):
            return False
    return True


def _quarantine_isolated_columns(
    ...
    table's observations, and is nearly empty or headerless — either through
    positional "Column X" fallback headers, or by never labelling itself on the
    worksheet at all.
    """
    ...
    for table_id in table_ids.unique():
        ...
        for cluster, cluster_obs in zip(clusters, cluster_sizes):
            ...
            unlabeled = _cluster_columns_lack_own_header(
                raw_df,
                cluster,
                int(member_rows.min()),
            )
            if sparse_fill or headerless or unlabeled:
...
def auto_flatten_report_tables(raw_df: pd.DataFrame, extraction_profile: str = "auto") -> pd.DataFrame:
    ...
    if extracted_regions:
        ...
        combined = _quarantine_isolated_columns(combined, raw_df)
    ...
    fallback = _quarantine_isolated_columns(fallback, raw_df)
...
def _enrich_hierarchy_from_totals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _as_series(column_value) -> pd.Series:
        # Duplicate column names can return a DataFrame; use the first column
        # so downstream lookups always yield scalars.
        if isinstance(column_value, pd.DataFrame):
            series = column_value.iloc[:, 0]
        else:
            series = pd.Series(column_value)
        return series.reset_index(drop=True)

    values_all = pd.to_numeric(_as_series(df[value_source]), errors="coerce")
    labels_all = _series_cell_text(_as_series(df["line_item"]))
    positions = pd.Series(np.arange(len(df), dtype=float))
    if "source_row" in df.columns:
        source_rows = pd.to_numeric(_as_series(df["source_row"]), errors="coerce")
    ...
    series_groups: OrderedDict[tuple[str, ...], list[int]] = OrderedDict()
    for pos, row in enumerate(df[series_key_columns].itertuples(index=False, name=None)):
        key = tuple("" if pd.isna(value) else str(value) for value in row)
        series_groups.setdefault(key, []).append(pos)

    for series_id, series_positions_raw in enumerate(series_groups.values()):
        series_positions = np.asarray(series_positions_raw, dtype=int)
        if len(series_positions) < 3 or len(series_positions) > MAX_SERIES_ROWS:
            continue
        ordered_positions = sorted(series_positions, key=lambda pos: float(order_key.iloc[pos]))
        vals = [
            float(values_all.iloc[pos]) if pd.notna(values_all.iloc[pos]) else None
            for pos in ordered_positions
        ]
        labs = [cell_text(labels_all.iloc[pos]) for pos in ordered_positions]
...
def _normalize_period_metric_granularity(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize period-only metrics to a stable total-level label.

    When a row only carries a period token (for example "2025 A") and has no
    column_group, treat it as total-level to keep metric shape consistent with
    sibling rows that expose "segment | period" headers.
    """
    if df is None or df.empty or "metric" not in df.columns:
        return df

    working = df.copy()
    if "column_group" not in working.columns:
        working["column_group"] = ""

    metric_text = _series_cell_text(working["metric"])
    group_text = _series_cell_text(working["column_group"])
    period_only = (
        metric_text.ne("")
        & metric_text.map(_looks_like_date_or_period_text)
        & metric_text.str.contains("|", regex=False).eq(False)
    )
    missing_group = group_text.eq("")
    fix_mask = period_only & missing_group
    if not bool(fix_mask.any()):
        return working

    working["metric"] = [
        f"{metric} | Total" if bool(apply_fix) else metric
        for metric, apply_fix in zip(metric_text.tolist(), fix_mask.tolist())
    ]
    working["column_group"] = [
        "Total" if bool(apply_fix) else group
        for group, apply_fix in zip(group_text.tolist(), fix_mask.tolist())
    ]
    return working
...
def finalize_extracted_sheet(df: pd.DataFrame, strip_text: bool = True, split_hierarchy: bool = True) -> pd.DataFrame:
    ...
    cleaned = _normalize_period_metric_granularity(cleaned)
    ...
    cleaned = _drop_block_bookkeeping_columns(cleaned)
...
def _standalone_process_excel(
    ...
    Not cached intentionally: callers may pass a progress callback that renders
    Streamlit elements, which is incompatible with cached-effect replay.
    """
