        ...
        for sheet_name in sheet_names:
            try:
                if is_sheet_already_flat(file_bytes, sheet_name):
                    st.write(f"⚡ Sheet '{sheet_name}' already flat - using fast read")
                else:
                    st.write(f"🔄 Flattening sheet (deterministic): {sheet_name}")
            except Exception:
                st.write(f"🔄 Processing sheet (deterministic): {sheet_name}")

            try:
                structured_df = extract_and_flatten_sheet(file_bytes, sheet_name, preferred_profile="auto")
            except Exception as e:
...
def load_data(uploaded_files: list[Any] | None, selected_sheets=None, convert_to_flat=False):
            ...
            for uploaded_file in uploaded_files:
                ...
                if convert_to_flat and file_extension in ['.xlsx', '.xls']:
                    ...
                    for sheet_name in sheets_to_process:
                        try:
                            if is_sheet_already_flat(file_bytes, sheet_name):
                                st.write(f"⚡ {sheet_name}: already flat (fast path)")
                            else:
                        ...
                        try:
                            structured_df = extract_and_flatten_sheet(
                                file_bytes,
                                sheet_name,
...
def load_uploaded_files(uploaded_files, selected_sheets=None):
    """Load uploaded files and initialize SQL agent"""
    if uploaded_files:
        # Load data with sheet selection and conversion option
        files_db = load_data(
            uploaded_files,
            selected_sheets,
        ...
        if uploaded_files and not st.session_state.files_loaded:
            ...
            if st.button("📋 Analyze Files", type="secondary", width="stretch"):
                with st.spinner("Analyzing files..."):
                    file_sheets = analyze_uploaded_files(uploaded_files)
                    st.session_state.file_sheets = file_sheets
                    # Initialize selected sheets with all sheets
            ...
            if st.session_state.show_sheet_selector and st.session_state.file_sheets:
                ...
                st.markdown("---")
                st.info("ℹ️ Excel sheets are automatically flattened and loaded as separate tables (one per selected tab).")

                # Load button
                st.markdown("---")
                total_selected = sum(len(sheets) for sheets in st.session_state.selected_sheets.values())
                if st.button(f"🔄 Load Files ({total_selected} sheet(s) selected)", type="primary", width="stretch"):
                    with st.spinner("Loading files..."):
                        if load_uploaded_files(uploaded_files, st.session_state.selected_sheets):
